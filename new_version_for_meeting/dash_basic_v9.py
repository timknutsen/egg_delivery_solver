...
def update_results(n_clicks, dummy_filter, order_data, fish_group_data, current_results):
    ctx = dash.callback_context
    triggered_id = ctx.triggered[0]['prop_id'].split('.')[0] if ctx.triggered else None
    ...
    if triggered_id == "filter-dummy-toggle" and current_results:
        ...
        return dash.no_update, display_results.to_dict("records"), results_cols, dash.no_update, dash.no_update, dash.no_update, dash.no_update
    ...
    if triggered_id == "solve-button" and n_clicks > 0:
        ...
        return (
            ...
            display_results.to_dict("records"),
            results_cols, 
...
def export_results_excel(n_clicks, results_data, capacity_data):
    if n_clicks:
        ...
        output.seek(0)
        return dcc.send_bytes(output.read(), "allocation_results.xlsx")
