...
def create_visualization(batches_df, results):
    ...
    try:
        ...
        # ** FIX STARTS HERE **
        # Add vertical lines and annotations separately to avoid the bug
        for _, order in results.iterrows():
            ...
            fig.add_annotation(
                x=delivery_date,
                y=1.05, # Position annotation just above the top of the plot area
                yref="paper", # 'paper' coordinates are relative to the plotting area
                text=f"Order {order['OrderNr']}",
                showarrow=False,
        ...
        fig.update_layout(
            title="Batch-Level Delivery Window Analysis", 
            xaxis_title="Date", 
            yaxis_title="Fish Group", 
            yaxis=dict(
                tickmode='array', 
                tickvals=list(y_positions.values()), 
                ticktext=list(y_positions.keys()),
                range=y_range # Ensure y-axis range is set
            ), 
            height=600, 
            hovermode='closest', 
            showlegend=True, 
...
def run_allocation_report(n_clicks):
    ...
    # 6. Assemble and return the results layout
    return html.Div([
        dbc.Card([
