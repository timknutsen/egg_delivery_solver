...
def calculate_degree_days_feasibility(
    ...
    Args:
        stripping_start: Date when stripping period begins (earliest eggs)
        stripping_stop: Date when stripping period ends (latest eggs)
    ...
    # At delivery date:
    # - Earliest eggs (stripped at stripping_start) have accumulated the MOST degree-days
    # - Latest eggs (stripped at stripping_stop) have accumulated the LEAST degree-days
    ...
    if delivery_date < stripping_stop:
        return False  # Cannot deliver roe that hasn't been stripped yet
    
    earliest_eggs_dd = days_from_earliest_strip * water_temp  # Oldest eggs, most mature
    latest_eggs_dd = days_from_latest_strip * water_temp      # Youngest eggs, least mature
    
    # 🔴 FIXED: Constraint 2 - ALL eggs must meet customer requirements
    # Latest (youngest) eggs must have reached minimum customer requirement
    if latest_eggs_dd < min_temp_customer:
        return False  # Some eggs not mature enough for customer
    
    # Earliest (oldest) eggs must not exceed maximum customer requirement
    if earliest_eggs_dd > max_temp_customer:
        return False  # Some eggs too old for customer
    
    # 🔴 FIXED: Constraint 3 - ALL eggs must still be within production quality window
    # Latest (youngest) eggs must have reached minimum production quality
    if latest_eggs_dd < min_temp_prod:
        return False  # Some eggs not mature enough to sell
    
    # Earliest (oldest) eggs must not be overripe
    if earliest_eggs_dd > max_temp_prod:
        return False  # Some eggs are overripe
...
def calculate_delivery_window_dates(
    ...
    Returns:
        ...
        Returns (pd.NaT, pd.NaT) if no valid window exists
    """
    ...
    if min_temp_prod >= max_temp_prod:
        return (pd.NaT, pd.NaT)  # Invalid temperature range
    
    # 🔴 FIXED: Earliest delivery = when YOUNGEST eggs (stripped last) are ready
    # Use ceiling for safety - round up to ensure minimum quality
    ...
    # Use floor (int) - can sell up until this point
    ...
    if earliest_delivery > latest_delivery:
        # Window is impossible - stripping period is too long relative to quality window
        return (pd.NaT, pd.NaT)
...
def add_delivery_windows(fish_groups, water_temp=DEFAULT_WATER_TEMP):
    """
    🔴 FIXED: Add delivery window columns (EarliestDelivery and LatestDelivery) 
    to fish groups DataFrame using corrected calculation.
    ...
    # Apply corrected delivery window calculation
    windows = df.apply(
...
def build_feasible_pairs(active_orders, all_fish_groups):
    ...
    for i in active_orders.index:
        ...
        for j in all_fish_groups.index:
            ...
            # 🔴 FIXED: DEGREE-DAYS CONSTRAINT with corrected calculation
...
def diagnose_dummy(order_row, fish_groups):
    ...
    for _, row in fish_groups.iterrows():
        ...
        # 🔴 FIXED: Check degree-days feasibility with corrected logic
        is_feasible = calculate_degree_days_feasibility(
            row["StrippingStartDate"], row["StrippingStopDate"],
            row["MinTemp_prod"], row["MaxTemp_prod"],
            min_temp_customer, max_temp_customer,
...
def solve_egg_allocation(orders, fish_groups):
    ...
    🔴 FIXED: Delivery window calculation now uses correct logic.
    """
    ...
    if active_orders.empty:
        ...
        # 🔴 FIXED: Add delivery windows with corrected calculation
    ...
    # 🔴 FIXED: Build feasible pairs with corrected degree-days constraints
    ...
    # Separate capacity constraints for Gain and Shield
    for j in all_fish_groups.index:
...
app.layout = dbc.Container([
    html.H1("🔧 Fish Egg Allocation Solver (Degree-Days - DELIVERY WINDOW FIXED)", className="text-center my-4 py-3 bg-primary text-white rounded"),
...
def update_results(n_clicks, dummy_filter, order_data, fish_group_data, selected_groups, current_results):
    ...
    if triggered_id == "solve-button" and n_clicks > 0:
        ...
        alert_text = "; ".join(warnings) if warnings else "✅ Optimization completed successfully with CORRECTED degree-days logic!"
        alert_open = True
...
if __name__ == "__main__":
    ...
    print(f"🔧 Starting Degree-Days Model server (DELIVERY WINDOW FIXED) on {host}:{port} with debug={debug}")
